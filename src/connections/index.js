...
exports.acceptRequest = async function(theirEndpointDid, theirDid, requestNonce) {
    ...
    await sdk.keyForDid(await indy.pool.get(), await indy.wallet.get(), theirDid);
    ...
    //TODO: Check to see if pairwise exists
...
exports.sendAcknowledgement = async function(myDid, theirDid, theirEndpointDid) {
    await indy.crypto.sendAnonCryptedMessage(
        theirEndpointDid,
        await indy.crypto.buildAuthcryptedMessage(myDid, theirDid, MESSAGE_TYPES.ACKNOWLEDGE, 'Success')
    );
