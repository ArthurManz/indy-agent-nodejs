...
exports.setup = async function(config, genesisPoolTransactions) {
    ...
    let poolGenesisTxnPath = await exports.getPoolGenesisTxnPath(config, genesisPoolTransactions);
    let poolConfig = {
...
exports.getPoolGenesisTxnPath = async function(config, genesisPoolTransactions) {
    let path = `${os.tmpdir()}/indy/${config.poolName}.txn`;
    await savePoolGenesisTxnFile(config, path, genesisPoolTransactions);
    return path;
};

async function poolGenesisTxnData(config) {
...
async function savePoolGenesisTxnFile(config, filePath, genesisPoolTransactions) {
    let data = genesisPoolTransactions;
    if (!data || data == null) data = await poolGenesisTxnData(config);
...
exports.getLedgerTransactions = async function(poolHandle, walletHandle, Did, from, to, type) {
    const response = [];
    for (let i = from; i < to; i++) {
        let request = await sdk.buildGetTxnRequest(Did, type.toUpperCase(), i);
        let result = await sdk.signAndSubmitRequest(poolHandle, walletHandle, Did, request);
